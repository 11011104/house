...
public class UserController {
	...
	public Map<String,Object> login(@RequestBody LoginUser loginUser) {
		...
		UserList userInfor = userService.findUserInfoByCondition(null,user.getId(),null);
		//生成令牌
		JwtUtil jwtUtil = new JwtUtil();
		String token = null;
		if(userInfor.getType() == 1){
			map.put("systemRole","admin");
			token = jwtUtil.createJWT(String.valueOf(userInfor.getId()),user.getUsername(),"admin");

		}else {
			map.put("systemRole","user");
			token = jwtUtil.createJWT(String.valueOf(userInfor.getId()),user.getUsername(),"user");
		}
		map.put("userInfo",userInfor);
		map.put("token",token);
		map.put("flag",true);
		return map;
	}
